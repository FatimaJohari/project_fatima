...
class Forecasting:
    ...
    def __init__(self, X,y ):
        self.X = int(PrepareData.X)
        self.y =int( PrepareData.y)
    ...
    def forecasting (self):
        train_X, test_X, train_y, test_y = Forecasting.trainTest(self)
        model = Sequential()
        model.add(LSTM(2, activation='relu', return_sequences=True, 
                       input_shape=(PrepareData.history, 1)))
        ...
        # fit network
        history = model.fit(Forecasting.train_X, Forecasting.train_y,
                            epochs=20, batch_size= 1, 
                            validation_data=(Forecasting.test_X, Forecasting.test_y),
                            verbose=2)
        return history
